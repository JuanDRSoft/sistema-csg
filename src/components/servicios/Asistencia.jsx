...
const Asistencia = ({ isOpen, closeModal }) => {
  const { clients } = useClient();

  const handleSubmit = () => {
    toast.info(
      "Si vas a cambiar de equipo o de Sim Card, modifica los datos del GPS"
    );
  };
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 px-8 text-left align-middle shadow-xl transition-all">
                ...
                <div className="mt-2">
                  <p className="mb-5">Que cliente deseas asistir:</p>

                  <div>
                    {clients.map((e) => (
                      <Link
                        to={`/app/clientes/${e.id}?service=asistencia`}
